    ...
    try {
        if (m.mentionedJid.includes(this.user.jid) && m.isGroup) {
            await this.send2Button(m.chat,
                isBanned ? 'SQUAT BOT NEXT PROJECT NOT ACTIVE' : banned ? 'TELAH DI BANNED' : 'SQUAT BOT NEXT PROJECT IN HERE',
                '©SQUAT BOT',
...
`.trim(), '©SQUAT BOT', 'CHAT ME TO BUY', ',owner', { contextInfo: { mentionedJid: [global.owner[0] + '@s.whatsapp.net'] } })
    }

    // salam
    let reg = /(ass?alam[assalamuaikum|waalaikumsalam]
